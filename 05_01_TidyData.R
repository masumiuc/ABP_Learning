...
# Tidy the data
tidy_ts <- sunspots %>%  
  as_tsibble() %>%        # Convert to time series tibble
...
tidy_ts %>%
  mutate(
    date = as.Date(yearmonth(date)),       # Get data
    decade = floor_date(date, "10 years")  # Get decade
  ) %>%
  group_by(decade) %>%                     # Group by decade
  summarize(mean_s = mean(spots,           # Calculate mean
    na.rm = TRUE)                          # Remove NAs
  ) %>% 
  ggplot(aes(decade, mean_s)) +            # Plot means
    geom_point() +                         # Scatterplot
    geom_smooth(method = "loess") +        # Smoother
    ylab("Sunspots: Mean by Decade")       # Y-axis label
